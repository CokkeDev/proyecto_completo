...
Evalúa tres métodos y genera benchmarking comparativo:
...
class SystemEvaluator:
    ...
    Ejecuta los tres clasificadores sobre el ground truth y genera:
      - Métricas por método
    ...
    def run_full_evaluation(self) -> dict:
        """
        Ejecuta evaluación de los 3 métodos y retorna reporte JSON completo.
        """
        ...
        y_pred_hybrid = self._predict_hybrid(entries, hybrid_clf)
        metrics_hybrid = self.calc.classification_metrics(y_true, y_pred_hybrid, all_classes)

        # ── Benchmarking ──────────────────────────────────────────────────────
        benchmark_results = [
            BenchmarkResult(method="rules", classification=metrics_rules),
            BenchmarkResult(method="embeddings", classification=metrics_emb),
            BenchmarkResult(method="hybrid", classification=metrics_hybrid),
        ]
        ...
        full_report = {
            ...
            "methods": {
                ...
                "hybrid": self._metrics_to_dict(metrics_hybrid),
            },
            "benchmark": report,
        }
    ...
    def _predict_hybrid(self, entries, clf: HybridClassifier) -> list[list[str]]:
        results = []
        for e in entries:
            ...
            labels = [result.primary_category]
            # Agregar otras categorías con score alto
            for cat, sc in result.hybrid_scores.items():
                if sc >= self.threshold and cat != result.primary_category:
                    labels.append(cat)
            results.append(labels[:5])
        return results
