...
TAXONOMY: dict = {
        ...
        "subcategorias": {
            ...
            "SEGURIDAD_SOCIAL": {
                ...
                "keywords": [
                    ...
                    "seguridad social", "beneficio previsional", "subsidio previsional",
                ],
                ...
                "reglas_semanticas": [
                    ...
                    r"(pensi[oó]n(es)?|jubilaci[oó]n(es)?)",
                    r"(sistema\s+previsional|reforma\s+previsional)",
                    r"(cotizaci[oó]n(es)?\s+(obligatoria|previsional))",
                    r"(defensor[ií]a\s+previsional)",
                    r"(sistema\s+de\s+pensiones)",
                    r"(reforma\s+previsional)",
                    r"(cotizaciones?\s+obligatorias)",
                    r"(derechos?\s+previsionales)",
                    r"(beneficios?\s+previsionales)",
                    r"(afiliados?\s+al\s+sistema)",
                ],
            },
            "SALUD_OCUPACIONAL": {
                ...
                "ejemplos_negativos": [
                    ...
                    "Modifica la ley orgánica del Congreso Nacional",
                    "Regula procedimientos administrativos del Estado",
                    "Establece normas de fiscalización parlamentaria",
                ],
                "reglas_semanticas": [
                    r"accidente\s+del\s+trabajo",
                    r"enfermedad\s+profesional",
                    r"\b(ACHS|IST|MUSEG)\b",
                    r"prevención\s+de\s+riesgos\s+laborales",
                    r"\bseguridad\s+(laboral|ocupacional)\b"
                    r"riesgos\s+laborales",
                    r"faena(s)?\s+(minera(s)?|industrial(es)?)",
                    r"exposición\s+(laboral|a\s+agentes)",
                ]
            },
            "SINDICALISMO": {
        ...
        "subcategorias": {
            "EDUCACION_ESCOLAR": {
                ...
                "keywords": [
                    ...
                    "docente", "profesor", "profesora", "asistente de la educación",
                ...
                "reglas_semanticas": [
                    ...
                    r"docente(s)?|profesor(es|as)?|asistente(s)?\s+de\s+la\s+educación",
                    r"convivencia\s+escolar|violencia\s+escolar|bullying",
                    r"(educaci[oó]n\s+(b[aá]sica|media))",
                    r"(alumno(s)?|estudiante(s)?)",
                    r"(convivencia\s+escolar|bullying)",
                ],
            },
            "EDUCACION_SUPERIOR": {
            ...
            "FORMACION_TECNICA": {
                ...
                "reglas_semanticas": [
                    ...
                    r"(centro\s+de\s+formaci[oó]n\s+t[eé]cnica)",
                    r"(educaci[oó]n\s+t[eé]cnico\s+profesional)",
                    r"(institutos?\s+tecnicos?)",
                    r"(capacitación\s+laboral|formación\s+profesional)",
                    r"\bcft\b",
                    r"(formaci[oó]n\s+t[eé]cnica)",
                    r"(educaci[oó]n\s+t[eé]cnico[-\s]profesional)",
                    r"(capacitaci[oó]n\s+laboral)",
                ],
            },
        },
    },
        ...
        "subcategorias": {
            "CAMBIO_CLIMATICO": {
                ...
                "keywords": [
                    ...
                    "ley marco de cambio climático", "mitigación climática", "adaptación climática",
                    "resiliencia climática", "transición energética",
                ],
                ...
                "reglas_semanticas": [
                    ...
                    r"ley\s+marco\s+de\s+cambio\s+climático|adaptación\s+climática|mitigación\s+climática",
                    r"(cambio\s+climatico)",
                    r"(calentamiento\s+global)",
                    r"(emisiones?\s+de\s+carbono)",
                    r"(gases?\s+de\s+efecto\s+invernadero)",
                    r"(huella\s+de\s+carbono)",
                ],
            },
            "BIODIVERSIDAD": {
                ...
                "keywords": [
                    ...
                    "área protegida", "áreas protegidas", "sitio prioritario", "sitios prioritarios",
                    "Sistema Nacional de Áreas Protegidas", "SNAP", "SBAP",
                    "Ley 21.600", "ley N° 21.600",
                    "CONAF", "SAG", "Servicio de Biodiversidad", "Servicio de Biodiversidad y Áreas Protegidas",
                    "tráfico de fauna", "caza furtiva", "pesca ilegal",
                    "turba", "turbera", "conservación de especies",
                ],
        ...
        "subcategorias": {
            ...
            "PROCEDIMIENTO_PENAL": {
                ...
                "reglas_semanticas": [
                    ...
                    r"audiencia\s+de\s+(formalización|control\s+de\s+detención)",
                    r"procedimiento\s+(abreviado|simplificado|penal)",
                ],
            },
            "CRIMEN_ORGANIZADO": {
    ...
    "ECONOMIA_FINANZAS": {
        ...
        "keywords": [
            ...
            "consumidor", "proveedor", "boleta", "factura", "transacción comercial",
            "ley 19.496", "protección de los derechos del consumidor",
        ],
        ...
        "subcategorias": {
            ...
            "PYMES_EMPRENDIMIENTO": {
                ...
                "keywords": [
                    ...
                    "patente municipal", "feria libre", "mercado", 
                    "matadero", "vega", "comerciante", "puesto", 
                    "copropiedad", "reglamento de copropiedad",
                    "emprendimiento local","actividad comercial",
                    "comercio minorista",
                ],
                ...
                "reglas_semanticas": [
                    ...
                    r"patente\s+municipal|feria(s)?\s+libre(s)?|mercado(s)?|matadero(s)?|vega(s)?",
                    r"pequeñ[oa]s?\s+comerciante(s)?",
                    r"actividad(es)?\s+comercial(es)?",
                    r"emprendimiento(s)?\s+local(es)?"
                ],
            },
            "PRESUPUESTO_FISCAL": {
            ...
            "PATENTES_MUNICIPALES": {
                "label": "Patentes municipales y comercio local",
                "definition": "Regulación de patentes comerciales otorgadas por municipalidades, especialmente en ferias, mercados y espacios de copropiedad.",
                ...
                "reglas_semanticas": [
                    r"patente(s)?\s+municipal(es)?",
                    r"renovaci[oó]n\s+de\s+patente(s)?",
                    r"feria(s)?\s+libre(s)?",
                    r"mercado(s)?\s+municipal(es)?",
                    r"matadero(s)?",
                    r"vega(s)?",
                    r"permiso(s)?\s+municipal(es)?"
                ],

                "ejemplos_positivos": [
                    "Regula la renovación de patentes municipales en ferias libres",
                    "Establece normas para el funcionamiento de mercados municipales",
                ],

                "ejemplos_negativos": [
                    "Modifica el impuesto a la renta",
                    "Regula el IVA en servicios digitales",
                    "proteccion de datos personales",
                    "tratamiento de datos",
                    "contrato de trabajo",
                    "derechos laborales",
            ...
            "REGULACION_OPERATIVA": {
                "label": "Regulación operativa de actividades económicas",
                "definition": "Normas de funcionamiento, administración y operación de actividades económicas reguladas.",
                ...
                "reglas_semanticas": [
                    r"reglamento(s)?\s+(interno(s)?|de\s+copropiedad)",
                    r"normas?\s+de\s+funcionamiento",
                    r"administraci[oó]n\s+(del|de\s+la)"
                ],

                "ejemplos_positivos": [
                    "Establece reglamento de copropiedad en mercados",
                    "Define normas de funcionamiento de ferias libres",
                ],

                "ejemplos_negativos": [
                    "Regula contratos de trabajo",
    ...
    "DERECHOS_FUNDAMENTALES": {
        ...
        "keywords": [
            ...
            "vida privada", "honra", "intimidad", "derecho a la imagen",
            "garantías constitucionales", "igualdad ante la ley",
            "libertad de expresión", "libertad de prensa", "libertad de información", "religión",
        ],
        ...
        "subcategorias": {
            ...
            "DERECHOS_MUJER_GENERO": {
                ...
                "reglas_semanticas": [
                    ...
                    r"(violencia\s+de\s+genero)",
                    r"(igualdad\s+de\s+genero)",
                    r"(equidad\s+de\s+genero)",
                    r"(derechos?\s+de\s+la\s+mujer)",
                    r"(violencia\s+intrafamiliar)",
                    r"(protecci[oó]n\s+de\s+la\s+mujer)",
                    r"(violencia\s+contra\s+la\s+mujer)",
                ],
            },
            "PUEBLOS_INDIGENAS": {
            ...
            "LIBERTADES_CIVILES": {
                ...
                "keywords": [
                    ...
                    "privacidad", "vida privada", "honra", "derecho a la imagen", "intimidad",
                    "datos personales", "hábeas corpus",
                ...
                "reglas_semanticas": [
                    ...
                    r"privacidad|vida\s+privada|datos\s+personales|derecho\s+a\s+la\s+imagen|honra|intimidad",
                    r"laicidad|libertad\s+religiosa",
                    r"(vida\s+privada)",
                    r"(derecho\s+a\s+la\s+imagen)",
                    r"(libertad\s+de\s+expresi[oó]n)",
                    r"(honra)",
                ],
            },
        },
        ...
        "subcategorias": {
            ...
            "PLANIFICACION_TERRITORIAL": {
                ...
                "keywords": [
                    ...
                    "ordenamiento territorial", "instrumento de planificación territorial",
                    "plan intercomunal", "plan seccional",
                ],
                ...
                "reglas_semanticas": [
                    ...
                    r"densificación|urbanización|ordenamiento\s+territorial",
                    r"instrumento(s)?\s+de\s+planificación\s+territorial",
                    r"permiso\s+de\s+(edificación|construcción)",
                    r"(plan\s+regulador)",
                    r"(uso\s+de\s+suelo)",
                    r"(zonificaci[oó]n)",
                    r"(ordenamiento\s+territorial)",
                ],
            },
            "SERVICIOS_BASICOS": {
...
"TECNOLOGIA_INNOVACION": {
    ...
    "definition": "Protección de datos personales, ciberseguridad, delitos informáticos, telecomunicaciones, inteligencia artificial e innovación tecnológica.",
    ...
    "prototype_texts": [
        "Modifica la ley N° 19.628 sobre protección de datos personales.",
        "Tipifica nuevos delitos informáticos y regula la ciberseguridad.",
        "Regula el uso de inteligencia artificial en el sector público.",
        "Moderniza los servicios digitales del Estado mediante firma electrónica."
    ],
    ...
    "subcategorias": {
        ...
        "DATOS_PERSONALES": {
            ...
            "definition": "Regulación del tratamiento de datos personales, privacidad digital y derechos asociados.",
            ...
            "ejemplos_positivos": [
                ...
                "Regula el tratamiento de datos biométricos"
            ],

            "ejemplos_negativos": [
                "Tipifica delitos informáticos",
                "Regula inteligencia artificial",
                "Reconoce el derecho a la vida privada sin datos"
            ],

            "reglas_semanticas": [
                r"protecci[oó]n\s+de\s+datos\s+personales",
                r"tratamiento\s+de\s+datos\s+personales",
                r"datos\s+personales(\s+(sensibles|biom[eé]tricos))?",
                r"datos\s+(sensibles|biom[eé]tricos)",
                r"ley\s+(n[°º]\s*)?19\.628",
                r"agencia\s+de\s+protecci[oó]n\s+de\s+datos(\s+personales)?",
                r"derechos?\s+ARCO",
                r"transferencia\s+internacional\s+de\s+datos\s+personales",
                r"privacidad\s+digital"
            ],
        },

        "CIBERSEGURIDAD": {
            "label": "Ciberseguridad y delitos informáticos",
            "definition": "Delitos informáticos, ataques cibernéticos y protección de sistemas informáticos.",
            ...
            "reglas_semanticas": [
                r"ciberseguridad",
                r"seguridad\s+inform[aá]tica",
                r"delitos?\s+inform[aá]ticos?",
                r"ataques?\s+cibern[eé]ticos?",
                r"phishing|ransomware|malware",
                r"acceso\s+indebido\s+a\s+sistemas"
            ],
        },

        "INTELIGENCIA_ARTIFICIAL": {
            "label": "Inteligencia artificial y automatización",
            "definition": "Uso de IA, algoritmos y decisiones automatizadas.",
            ...
            "reglas_semanticas": [
                ...
                r"algoritmo(s)?\s+(automatizado(s)?|predictivo(s)?)",
                r"decisiones?\s+automatizadas?",
                r"sistema\s+algor[ií]tmico"
            ],
        },

        "TELECOMUNICACIONES": {
            "label": "Telecomunicaciones e infraestructura digital",
            "definition": "Regulación de redes, conectividad y servicios de telecomunicaciones.",
        ...
        "subcategorias": {
            ...
            "FUNCION_PUBLICA": {
                ...
                "reglas_semanticas": [
                    ...
                    r"funci[oó]n\s+p[uú]blica",
                    r"empleados?\s+p[uú]blicos?",
                    r"sector\s+p[uú]blico",
                ],
            },
            "GESTION_PUBLICA": {
                "label": "Gestión pública y administración del Estado",
                "definition": "Modernización del Estado, eficiencia administrativa, gestión interna de organismos públicos y políticas de administración pública.",
                "keywords": [
                    "gestión pública", "administración pública", "modernización del Estado",
                    "eficiencia administrativa", "servicio público", "función pública",
                    "organismo público", "sector público", "gestión estatal","municipalidad",
                    "municipal","fiscalizacion municipal","autoridad comunal","administracion municipal"
                ],
                "synonyms": ["administración estatal", "gestión estatal"],
                "etiquetas": ["gestion_publica", "modernizacion_estado", "administracion_publica"],
                "ejemplos_positivos": [
                    "Modifica normas para mejorar la gestión de los servicios públicos",
                    "Moderniza la administración del Estado",
                ],
                "ejemplos_negativos": [
                    "Regula el sistema de pensiones",
                ...
                "reglas_semanticas": [
                    r"gestión\s+pública|administración\s+pública",
                    r"modernización\s+del\s+estado",
                    r"sector\s+público"
                    r"municipalidad(es)?",
                    r"autoridad(es)?\s+municipal(es)?",
                    r"fiscalizaci[oó]n\s+municipal",
                ],
            },
            "GOBIERNO_DIGITAL": {
                "label": "Gobierno digital y tecnologías del Estado",
                "definition": "Uso de tecnologías digitales en el Estado, digitalización de servicios públicos y transformación digital gubernamental.",
                "keywords": [
                    "gobierno digital", "transformación digital", "servicios digitales",
                    "plataformas digitales del Estado", "digitalización",
                    "trámites en línea", "Estado digital"
                ],
                "synonyms": ["estado digital", "digitalización pública"],
                "etiquetas": ["gobierno_digital", "digitalizacion", "servicios_digitales"],
                "ejemplos_positivos": [
                    "Regula el uso de plataformas digitales en el Estado",
                    "Establece el gobierno digital"
                ],
                "ejemplos_negativos": [
                ...
                "reglas_semanticas": [
                    r"gobierno\s+digital",
                    r"transformación\s+digital",
                    r"servicios?\s+digitales?\s+(del\s+estado)?"
                ],
            },
            "TRATADOS_INTERNACIONALES": {
                "label": "Tratados y acuerdos internacionales",
                "definition": "Aprobación de tratados, convenios internacionales y acuerdos bilaterales o multilaterales.",
                "keywords": [
                    "tratado internacional", "acuerdo internacional",
                    "convenio internacional", "ratificación",
                    "acuerdo bilateral", "acuerdo multilateral"
                ],
                "synonyms": ["tratados", "convenios internacionales"],
                "etiquetas": ["tratados", "acuerdos_internacionales"],
                "ejemplos_positivos": [
                    "Aprueba tratado internacional",
                    "Ratifica convenio internacional"
                ],
                "ejemplos_negativos": [
                    "Regula transporte público",
                    "Modifica el Código Penal"
                ],
                "reglas_semanticas": [
                    r"tratado\s+internacional",
            ...
            "PATRIMONIO_CULTURAL": {
                ...
                "definition": "Protección y reconocimiento del patrimonio cultural, tradiciones y expresiones culturales.",
                "keywords": [
                    "patrimonio cultural", "cultura", "tradición",
                    "identidad cultural", "expresiones culturales"
                ],
                "synonyms": ["cultura", "tradiciones"],
                "etiquetas": ["patrimonio", "cultura"],
                "ejemplos_positivos": [
                    "Declara patrimonio cultural",
                    "Reconoce tradición cultural"
                ],
                "ejemplos_negativos": [
                    "Regula impuestos",
                    "Modifica normas laborales"
            ...
            "CONMEMORACIONES_OFICIALES": {
                "label": "Conmemoraciones oficiales",
                "definition": "Establecimiento de días nacionales, efemérides y conmemoraciones oficiales.",
                "keywords": [
                    "día nacional", "conmemoración",
                    "efeméride", "celebración oficial"
                ],
                "synonyms": ["día conmemorativo"],
                "etiquetas": ["conmemoracion", "dia_nacional"],
                "ejemplos_positivos": [
                    "Establece día nacional",
                    "Declara efeméride"
                ],
                "ejemplos_negativos": [
                    "Regula transporte",
    ...
    "definition": "Regulación del transporte terrestre, aéreo y marítimo, tránsito, seguridad vial, licencias de conducir, transporte público y operación de servicios de transporte.",
    "keywords": [
        "transporte", "tránsito", "movilidad", "vehículo", "conductor",
        "licencia de conducir", "seguridad vial", "Ley de Tránsito",
        "transporte público", "buses", "metro", "taxis", "colectivos",
        "transporte aéreo", "aerolínea", "pasajes", "transporte marítimo",
        "registro de transporte", "vías exclusivas", "infracción de tránsito",
        "control de velocidad", "alcoholímetro", "drogas en conducción"
    ],
    "synonyms": ["movilidad", "tránsito", "sistema de transporte"],
    "prototype_texts": [
        "Modifica la ley de tránsito para establecer nuevas normas de seguridad vial.",
        "Regula el transporte público de pasajeros y sus condiciones de operación.",
        "Establece requisitos para la obtención de licencias de conducir.",
        "Regula el transporte aéreo y los derechos de los pasajeros."
    ],

    "subcategorias": {

        "SEGURIDAD_VIAL": {
            "label": "Seguridad vial y conducción",
            "definition": "Normas sobre conducción segura, control de alcohol y drogas, infracciones de tránsito, accidentes y prevención vial.",
            "keywords": [
                "seguridad vial", "conducción", "accidente de tránsito",
                "alcoholímetro", "narcotest", "control de drogas",
                "exceso de velocidad", "infracción de tránsito",
                "licencia suspendida", "conducción temeraria",
                "normas de tránsito", "señalización vial"
            ],
            "synonyms": ["prevención vial", "seguridad en carreteras"],
            "etiquetas": ["seguridad_vial", "infracciones", "control_drogas", "accidentes"],
            "reglas_semanticas": [
                r"seguridad\s+vial",
                r"conducción\s+(temeraria|bajo\s+la\s+influencia)",
                r"alcoholímetro|narcotest|drogas\s+en\s+conducción",
                r"infracción(es)?\s+de\s+tránsito",
                r"accidente(s)?\s+de\s+tránsito"
            ],
        },

        "TRANSPORTE_PUBLICO": {
            "label": "Transporte público de pasajeros",
            "definition": "Regulación de buses, metro, taxis, transporte escolar y condiciones del servicio de transporte público.",
            "keywords": [
                "transporte público", "bus", "micro", "metro",
                "taxis", "colectivos", "transporte escolar",
                "pasajeros", "servicio de transporte",
                "tarifa de transporte", "empresa de transporte"
            ],
        ...
        "LICENCIAS_CONDUCIR": {
            "label": "Licencias de conducir y requisitos",
            "definition": "Regulación de licencias de conducir, requisitos físicos y psicológicos, exámenes y certificaciones.",
            "keywords": [
                "licencia de conducir", "permiso de conducir",
                "idoneidad física", "idoneidad psíquica",
                "examen de conducir", "certificación médica",
                "obtención de licencia"
            ],
            "synonyms": ["permiso de conducción"],
            "etiquetas": ["licencias", "conductores", "requisitos"],
            "reglas_semanticas": [
                r"licencia\s+de\s+conducir",
                r"permiso\s+de\s+conducir",
                r"idoneidad\s+(física|psíquica)",
                r"examen\s+de\s+conducir"
            ],
        },

        "REGULACION_OPERATIVA": {
            "label": "Regulación operativa del transporte",
            "definition": "Regulación de registros, inscripciones, operación de servicios y obligaciones de empresas de transporte.",
            "keywords": [
                "registro de transporte", "inscripción de vehículos",
                "registro nacional", "empresa de transporte",
                "nómina de pasajeros", "control de pasajeros",
                "operación del servicio"
            ],
            "synonyms": ["gestión del transporte"],
            "etiquetas": ["registro", "empresas_transporte", "control"],
            "reglas_semanticas": [
                r"registro\s+nacional\s+de\s+transporte",
                r"inscripción\s+de\s+vehículos",
                r"nómina\s+de\s+pasajeros",
                r"empresa(s)?\s+de\s+transporte"
                r"regula\s+la\s+aplicaci[oó]n",
                r"procedimiento\s+administrativo",
                r"normas\s+de\s+funcionamiento",
                r"reglamentaci[oó]n",
                r"obligaciones?\s+de\s+empresas?\s+de\s+transporte",
                r"operaci[oó]n\s+del\s+servicio",
            ],
        },

        "TRANSPORTE_AEREO_MARITIMO": {
            "label": "Transporte aéreo y marítimo",
            "definition": "Regulación del transporte aéreo, marítimo, aerolíneas, puertos y derechos de pasajeros.",
            "keywords": [
                "transporte aéreo", "aerolínea", "vuelo",
                "pasaje aéreo", "sobreventa", "overbooking",
                "transporte marítimo", "puertos", "navegación"
            ],
            "synonyms": ["aviación", "transporte marítimo"],
            "etiquetas": ["aereo", "maritimo", "pasajes"],
            "reglas_semanticas": [
                r"transporte\s+aéreo|vuelo(s)?",
                r"aerolínea(s)?|pasaje(s)?",
                r"overbooking|sobreventa",
                r"transporte\s+marítimo|puerto(s)?"
            ],
        },
        "TRANSPORTE_ESCOLAR": {
            "label": "Transporte escolar",
            "definition": "Regulación del transporte de estudiantes, registro de vehículos escolares y condiciones de operación.",
            "keywords": [
                "transporte escolar", "vehículos escolares",
                "registro de transporte escolar", "estudiantes"
            ],
            "synonyms": ["transporte de escolares"],
            "etiquetas": ["transporte_escolar"],
            "ejemplos_positivos": [
                "Regula transporte escolar",
                "Suspende registro de transporte escolar"
            ],
            "ejemplos_negativos": [
                "Regula transporte aéreo",
                "Modifica normas laborales"
            ],
            "reglas_semanticas": [
                r"transporte\s+escolar",
                r"registro\s+nacional\s+de\s+transporte\s+escolar"
            ],
        },
    },
},
...
# ─────────────────────────────────────────────────────────────
# Ajustes de optimización para evaluación H0
# Agregados para mejorar recall en subcategorías con F1 bajo o cero.
# Estos bloques no reemplazan la taxonomía base: extienden keywords,
# synonyms y reglas_semanticas preservando la estructura original.
# ─────────────────────────────────────────────────────────────

_EXTRA_TAXONOMY_RULES = {
    "DERECHO_LABORAL_EMPLEO": {
        "keywords": [
            "permiso laboral", "fuero laboral", "protección del empleo",
            "derechos laborales", "relación laboral", "Código del Trabajo"
        ],
        "subcategorias": {
            "SEGURIDAD_SOCIAL": {
                "keywords": [
                    "seguridad social", "previsión social", "sistema previsional",
                    "reforma previsional", "cotización previsional", "cotizaciones previsionales",
                    "pensión garantizada universal", "PGU", "pensión básica solidaria",
                    "aporte previsional solidario", "pilar solidario", "fondo de pensiones",
                    "fondos previsionales", "administradora de fondos de pensiones",
                    "AFP", "IPS", "AFC", "jubilación", "jubilado", "pensionado",
                    "pensión de vejez", "pensión de invalidez", "seguro de cesantía",
                    "beneficios previsionales", "capitalización individual"
                ],
                "synonyms": [
                    "pensiones", "beneficios previsionales", "régimen previsional",
                    "sistema de seguridad social"
                ],
                "reglas_semanticas": [
                    r"\b(AFP|IPS|AFC|PGU)\b",
                    r"seguridad\s+social|previsión\s+social|sistema\s+previsional",
                    r"reforma\s+previsional|cotizaciones?\s+previsionales?",
                    r"pensión\s+(garantizada\s+universal|básica\s+solidaria|de\s+vejez|de\s+invalidez|solidaria)",
                    r"fondos?\s+(previsionales?|de\s+pensiones)|administradora\s+de\s+fondos\s+de\s+pensiones",
                    r"seguro\s+de\s+cesantía|aporte\s+previsional\s+solidario|pilar\s+solidario",
                    r"capitalización\s+individual|beneficios?\s+previsionales?",
                    r"(pension(es)?|jubilacion(es)?)",
                    r"(sistema\s+previsional)",
                    r"(afp)",
                    r"(cotizacion(es)?\s+previsional(es)?)",
                    r"(reforma\s+previsional)",
                    r"(sistema\s+de\s+pensiones)",
                    r"(cotizaciones?\s+obligatorias)",
                    r"(beneficios?\s+previsionales)",
                    r"(afiliados?\s+al\s+sistema\s+de\s+pensiones)",
                    r"(instituciones?\s+de\s+seguridad\s+social)",
                    r"seguridad\s+social",
                    r"sistema\s+previsional",
                    r"pensiones?",
                    r"cotizaciones?",
                    r"afiliados?",
                    r"afiliados?\s+a\s+instituciones?\s+de\s+seguridad\s+social",
                    r"instituciones?\s+de\s+seguridad\s+social",
                    r"derechos?\s+y\s+deberes?\s+de\s+los\s+afiliados",
                    r"sistema\s+de\s+seguridad\s+social",
                ],
            },
            "SALUD_OCUPACIONAL": {
                "keywords": [
                    "salud ocupacional", "seguridad y salud en el trabajo", "riesgos laborales",
                    "prevención de riesgos laborales", "accidentes laborales", "enfermedades profesionales",
                    "condiciones de seguridad", "higiene y seguridad", "comité paritario",
                    "dirección del trabajo", "fiscalización laboral", "faena segura"
                ],
                "reglas_semanticas": [
                    r"salud\s+ocupacional|seguridad\s+y\s+salud\s+en\s+el\s+trabajo",
                    r"riesgos?\s+laborales?|prevención\s+de\s+riesgos\s+laborales?",
                    r"accidentes?\s+laborales?|enfermedades?\s+profesionales?",
                    r"comité\s+paritario|higiene\s+y\s+seguridad",
                    r"condiciones\s+de\s+seguridad\s+(laboral|en\s+el\s+trabajo)"
                    r"(salud\s+ocupacional)",
                    r"(riesgos?\s+laborales?)",
                    r"(accidentes?\s+del\s+trabajo)",
                    r"(seguridad\s+laboral)",
                    r"(seguridad\s+en\s+el\s+trabajo)",
                ],
            },
        },
    },
    "SALUD_PUBLICA": {
        "keywords": [
            "derecho a la salud", "acceso a la salud", "cobertura sanitaria",
            "prestaciones de salud", "sistema público de salud", "red asistencial"
        ],
        "subcategorias": {
            "COBERTURA_SANITARIA": {
                "keywords": [
                    "derecho a la salud", "acceso a la salud", "acceso oportuno a la salud",
                    "cobertura sanitaria", "cobertura universal", "salud universal",
                    "prestaciones de salud", "prestaciones garantizadas", "garantías explícitas en salud",
                    "GES", "AUGE", "FONASA", "ISAPRE", "red asistencial",
                    "sistema público de salud", "atención primaria de salud", "APS", "CESFAM",
                    "lista de espera", "listas de espera", "copago", "arancel FONASA",
                    "modalidad libre elección", "protección financiera en salud"
                ],
                "synonyms": [
                    "acceso sanitario", "cobertura de prestaciones", "aseguramiento en salud"
                ],
                "reglas_semanticas": [
                    r"derecho\s+a\s+la\s+salud|acceso\s+(oportuno\s+)?a\s+la\s+salud",
                    r"cobertura\s+(universal|sanitaria|de\s+salud)",
                    r"prestaciones?\s+(garantizadas?|de\s+salud)",
                    r"garantías\s+explícitas\s+en\s+salud|\bGES\b|\bAUGE\b",
                    r"\bFONASA\b|\bISAPRE\b|arancel\s+FONASA|modalidad\s+libre\s+elección",
                    r"red\s+asistencial|sistema\s+público\s+de\s+salud|atención\s+primaria\s+de\s+salud|\bCESFAM\b",
                    r"listas?\s+de\s+espera|protección\s+financiera\s+en\s+salud"
                    r"(acceso\s+a\s+la\s+salud)",
                    r"(prestaciones\s+de\s+salud)",
                    r"(garant[ií]as\s+expl[ií]citas)",
                ],
            },
            "REGULACION_PROFESIONAL_SALUD": {
                "keywords": [
                    "profesionales de la salud", "personal de salud", "funcionarios de salud",
                    "médicos cirujanos", "enfermeras", "matronas", "odontólogos", "kinesiólogos",
                    "tecnólogos médicos", "técnicos en enfermería", "TENS", "EUNACOM",
                    "carrera sanitaria", "carrera funcionaria de salud", "colegio médico",
                    "responsabilidad sanitaria", "responsabilidad médica", "negligencia médica",
                    "mala praxis", "establecimientos de salud", "prestadores de salud",
                    "acreditación hospitalaria", "habilitación profesional"
                ],
                "reglas_semanticas": [
                    r"profesionales?\s+de\s+la\s+salud|personal\s+de\s+salud|funcionarios?\s+de\s+salud",
                    r"médicos?\s+cirujanos?|enfermeras?|matronas?|odontólogos?|kinesiólogos?|tecnólogos?\s+médicos?",
                    r"\bTENS\b|técnicos?\s+en\s+enfermería|\bEUNACOM\b",
                    r"carrera\s+(sanitaria|funcionaria\s+de\s+salud)",
                    r"responsabilidad\s+(médica|sanitaria)|negligencia\s+médica|mala\s+praxis",
                    r"prestadores?\s+de\s+salud|establecimientos?\s+de\s+salud|acreditación\s+hospitalaria",
                    r"habilitación\s+profesional|colegio\s+médico"
                ],
            },
        },
    },
    "EDUCACION": {
        "keywords": [
            "sistema educativo", "ley general de educación", "educación escolar",
            "educación superior", "establecimientos educacionales"
        ],
        "subcategorias": {
            "EDUCACION_ESCOLAR": {
                "keywords": [
                    "educación escolar", "educación básica", "educación media", "educación parvularia",
                    "educación inicial", "establecimiento educacional", "establecimientos educacionales",
                    "comunidad educativa", "convivencia escolar", "reglamento interno escolar",
                    "alumnos", "estudiantes", "apoderados", "sostenedor", "sostenedores",
                    "subvención escolar", "jornada escolar completa", "JEC", "SAE",
                    "sistema de admisión escolar", "currículum nacional", "plan de estudios",
                    "asistencia escolar", "deserción escolar", "repitencia", "bullying",
                    "violencia escolar", "consejo escolar"
                ],
                "reglas_semanticas": [
                    r"educación\s+(escolar|básica|media|parvularia|inicial)",
                    r"establecimientos?\s+educacionales?|comunidad\s+educativa",
                    r"convivencia\s+escolar|reglamento\s+interno\s+escolar|violencia\s+escolar|bullying",
                    r"sistema\s+de\s+admisión\s+escolar|\bSAE\b|subvención\s+escolar",
                    r"jornada\s+escolar\s+completa|\bJEC\b",
                    r"curr[ií]culum\s+nacional|plan\s+de\s+estudios",
                    r"asistencia\s+escolar|deserción\s+escolar|repitencia"
                ],
            },
            "EDUCACION_SUPERIOR": {
            ...
            "FORMACION_TECNICA": {
                "keywords": [
                    ...
                    "liceo técnico profesional", "liceos técnicos", "formación dual",
                    "educación técnica", "capacitación técnica", "certificación de competencias laborales"
                ],
                "reglas_semanticas": [
                    r"educación\s+técnico[-\s]profesional|liceos?\s+técnico(s)?",
                    r"formación\s+dual|educación\s+técnica",
                    r"certificación\s+de\s+competencias\s+laborales?",
                    r"(formacion\s+tecnica)",
                    r"(educacion\s+tecnico\s+profesional)",
                    r"(capacitacion\s+laboral)",
                    r"(centro\s+de\s+formaci[oó]n\s+t[eé]cnica)",
                    r"\bcft\b",
                ],
            },
    ...
    "MEDIO_AMBIENTE": {
        "subcategorias": {
            ...
            "PLANIFICACION_TERRITORIAL": {
                "keywords": [
                    "planificación territorial", "ordenamiento territorial", "ordenamiento del territorio",
                    "plan regulador", "plan regulador comunal", "plan regulador intercomunal",
                    "instrumento de planificación territorial", "instrumentos de planificación territorial",
                    "uso de suelo", "cambio de uso de suelo", "zonificación", "zona urbana", "zona rural",
                    "límite urbano", "desarrollo urbano", "expansión urbana", "densificación urbana",
                    "subdivisión predial", "permisos de edificación", "dirección de obras municipales", "DOM"
                ],
                "reglas_semanticas": [
                    r"planificación\s+territorial|ordenamiento\s+(territorial|del\s+territorio)",
                    r"planes?\s+reguladores?\s+(comunales?|intercomunales?)?",
                    r"instrumentos?\s+de\s+planificación\s+territorial",
                    r"uso\s+de\s+suelo|cambio\s+de\s+uso\s+de\s+suelo|zonificación",
                    r"límite\s+urbano|zona\s+(urbana|rural)",
                    r"desarrollo\s+urbano|expansión\s+urbana|densificación\s+urbana",
                    r"subdivisión\s+predial|permisos?\s+de\s+edificación|dirección\s+de\s+obras\s+municipales|\bDOM\b"
                ],
            },
        },
    },
    "DERECHOS_FUNDAMENTALES": {
        "keywords": [
            "libertades civiles", "libertad personal", "vida privada", "honra", "intimidad",
            "derecho a la imagen", "protección de datos personales", "igualdad ante la ley",
            "derechos de la mujer", "igualdad de género", "paridad de género"
        ],
        "subcategorias": {
            "LIBERTADES_CIVILES": {
                "keywords": [
                    "libertad de expresión", "libertad de prensa", "libertad de información",
                    "libertad de opinión", "libertad de conciencia", "libertad religiosa",
                    "libertad personal", "seguridad individual", "vida privada", "honra",
                    "intimidad", "derecho a la imagen", "inviolabilidad del hogar",
                    "inviolabilidad de las comunicaciones", "protección de datos personales",
                    "datos personales", "autodeterminación informativa", "censura previa",
                    "secreto de las comunicaciones", "derecho de reunión", "derecho de asociación"
                ],
                "reglas_semanticas": [
                    r"libertad\s+(de\s+)?(expresión|prensa|información|opinión|conciencia|religiosa)",
                    r"libertad\s+personal|seguridad\s+individual",
                    r"vida\s+privada|honra|intimidad|derecho\s+a\s+la\s+imagen",
                    r"inviolabilidad\s+(del\s+hogar|de\s+las\s+comunicaciones)",
                    r"protección\s+de\s+datos\s+personales|datos\s+personales|autodeterminación\s+informativa",
                    r"censura\s+previa|secreto\s+de\s+las\s+comunicaciones",
                    r"derecho\s+de\s+(reunión|asociación)"
                ],
            },
            "DERECHOS_MUJER_GENERO": {
                "keywords": [
                    "derechos de la mujer", "igualdad de género", "equidad de género", "paridad de género",
                    "paridad", "violencia contra la mujer", "violencia de género", "SERNAMEG",
                    "SERNAM", "Ministerio de la Mujer", "brecha salarial", "igualdad salarial",
                    "salario igual", "maternidad", "corresponsabilidad parental", "cuidados",
                    "aborto", "interrupción voluntaria del embarazo", "IVE", "tres causales",
                    "cuotas de género", "representación equilibrada"
                ],
                "reglas_semanticas": [
                    r"derechos?\s+de\s+la\s+mujer|igualdad\s+(de\s+)?género|equidad\s+de\s+género",
                    r"paridad\s+(de\s+)?género|cuotas?\s+de\s+género|representación\s+equilibrada",
                    r"violencia\s+(contra\s+la\s+mujer|de\s+género)",
                    r"\bSERNAMEG\b|\bSERNAM\b|ministerio\s+de\s+la\s+mujer",
                    r"brecha\s+salarial|igualdad\s+salarial|salario\s+igual",
                    r"corresponsabilidad\s+parental|maternidad|cuidados",
                    r"\baborto\b|interrupción\s+(voluntaria\s+)?del\s+embarazo|\bIVE\b|tres\s+causales",
                    r"(violencia\s+intrafamiliar)",
                    r"(protecci[oó]n\s+de\s+la\s+mujer)",
                    r"(violencia\s+contra\s+la\s+mujer)",
                    r"(maltrato\s+contra\s+la\s+mujer)",
    ...
    "ECONOMIA_FINANZAS": {
        "subcategorias": {
            "SISTEMA_FINANCIERO": {
                "keywords": [
                    "mercado financiero", "mercado de valores", "servicios financieros",
                    "productos financieros", "crédito de consumo", "créditos de consumo",
                    "tarjetas de crédito", "tasa máxima convencional", "deudor", "deudores"
                ],
                "reglas_semanticas": [
                    r"mercado\s+financiero|mercado\s+de\s+valores|servicios\s+financieros",
                    r"productos\s+financieros|créditos?\s+de\s+consumo|tarjetas?\s+de\s+crédito",
                    r"tasa\s+máxima\s+convencional|sobreendeudamiento|deudores?"
                ],
            },
            "TRIBUTACION": {
                "keywords": [
                    "exención tributaria", "beneficio tributario", "crédito tributario",
                    "impuesto específico", "contribuciones", "patente municipal",
                    "régimen tributario", "cumplimiento tributario"
                ],
                "reglas_semanticas": [
                    r"exención\s+tributaria|beneficio\s+tributario|crédito\s+tributario",
                    r"impuesto\s+específico|contribuciones|patente\s+municipal",
                    r"régimen\s+tributario|cumplimiento\s+tributario"
                ],
            },
        },
    },
...
def _extend_unique_list(target: dict, key: str, values: list[str]) -> None:
    base = target.setdefault(key, [])
    for value in values:
        if value not in base:
            base.append(value)


def _merge_extra_taxonomy_rules() -> None:
    for cat_code, cat_extra in _EXTRA_TAXONOMY_RULES.items():
        if cat_code not in TAXONOMY:
            continue
        cat = TAXONOMY[cat_code]
        for list_key in ("keywords", "synonyms", "prototype_texts"):
            if list_key in cat_extra:
                _extend_unique_list(cat, list_key, cat_extra[list_key])
        for sub_code, sub_extra in cat_extra.get("subcategorias", {}).items():
            subcats = cat.setdefault("subcategorias", {})
            if sub_code not in subcats:
                continue
            subcat = subcats[sub_code]
            for list_key in ("keywords", "synonyms", "etiquetas", "ejemplos_positivos", "reglas_semanticas"):
                if list_key in sub_extra:
                    _extend_unique_list(subcat, list_key, sub_extra[list_key])


_merge_extra_taxonomy_rules()
